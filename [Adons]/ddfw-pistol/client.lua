...
RegisterNetEvent('ddfw-pistol:queueInfo')
AddEventHandler('ddfw-pistol:queueInfo', function(text, other)
    queueText = text
    matchends = other
end)

RegisterNetEvent('ddfw-pistol:hudNotify')
AddEventHandler('ddfw-pistol:hudNotify', function(text)
    SetNotificationTextEntry('STRING')
    AddTextComponentSubstringPlayerName(text)
    DrawNotification(false, true)
end)

RegisterNetEvent('ddfw-pistol:start')
AddEventHandler('ddfw-pistol:start', function()
    queuing = false 
...
RegisterNetEvent('ddfw-pistol:stop')
AddEventHandler('ddfw-pistol:stop', function()
...
RegisterNetEvent('ddfw-pistol:matchOver')
AddEventHandler('ddfw-pistol:matchOver', function(winner, me)
...
RegisterNetEvent('ddfw-pistol:died')
AddEventHandler('ddfw-pistol:died', function(killedBy)
    deaths = deaths + 1
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.JoinCircle, true) <= 7.5 then
                if queueText ~= Config.Translations['match_progress'] then
                    ...
                    if IsControlJustReleased(0, 38) then
                        queuing = not queuing
                        TriggerServerEvent('ddfw-pistol:join')
                    end
                else
                    ...
                    if GetEntityType(ped) == 1 then
                        for k, v in pairs(GetActivePlayers()) do
                            if GetPlayerPed(v) == ped then
                                TriggerServerEvent('ddfw-pistol:kill', GetPlayerServerId(v))
                                kills = kills + 1
                                Wait(500)
                                break
                            end
