...
if Config.EnableNitro then
    ...
    RegisterNetEvent('ddfw-hud:SetupNitro')
    AddEventHandler('ddfw-hud:SetupNitro', function()
        ...
        if IsPedSittingInAnyVehicle(playerPed) then
            ...
            if vehicle ~= nil and DoesEntityExist(vehicle) and IsPedOnFoot(playerPed) then
                ...
                Citizen.CreateThread(function()
                    ...
                    TriggerServerEvent('ddfw-hud:RemoveItem',Config.NitroItem, 1)
                    TriggerServerEvent('ddfw-hud:InstallNitro', GetVehicleNumberPlateText(vehicle))
                end)
            else
    ...
    RegisterNetEvent('ddfw-hud:UpdateNitroData')
    AddEventHandler('ddfw-hud:UpdateNitroData', function(nitroData)
        nitro = nitroData
                ...
                if GetPedInVehicleSeat(playerVeh, -1) == playerPed then
                    while isPressing do
                        ...
                        if tonumber(nitro[GetVehicleNumberPlateText(playerVeh)]) <= 0 then
                            ...
                            TriggerServerEvent('ddfw-hud:UpdateNitro', GetVehicleNumberPlateText(playerVeh), nitro[GetVehicleNumberPlateText(playerVeh)])
                            break
