...
RegisterNetEvent('ddfw-shotgun:queueInfo')
AddEventHandler('ddfw-shotgun:queueInfo', function(text, other)
    queueText = text
    matchends = other
end)

RegisterNetEvent('ddfw-shotgun:hudNotify')
AddEventHandler('ddfw-shotgun:hudNotify', function(text)
    SetNotificationTextEntry('STRING')
    AddTextComponentSubstringPlayerName(text)
    DrawNotification(false, true)
end)

RegisterNetEvent('ddfw-shotgun:start')
AddEventHandler('ddfw-shotgun:start', function()
    queuing = false 
...
RegisterNetEvent('ddfw-shotgun:stop')
AddEventHandler('ddfw-shotgun:stop', function()
...
RegisterNetEvent('ddfw-shotgun:matchOver')
AddEventHandler('ddfw-shotgun:matchOver', function(winner, me)
...
RegisterNetEvent('ddfw-shotgun:died')
AddEventHandler('ddfw-shotgun:died', function(killedBy)
    deaths = deaths + 1
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.JoinCircle, true) <= 7.5 then
                if queueText ~= Config.Translations['match_progress'] then
                    ...
                    if IsControlJustReleased(0, 38) then
                        queuing = not queuing
                        TriggerServerEvent('ddfw-shotgun:join')
                    end
                else
                    ...
                    if GetEntityType(ped) == 1 then
                        for k, v in pairs(GetActivePlayers()) do
                            if GetPlayerPed(v) == ped then
                                TriggerServerEvent('ddfw-shotgun:kill', GetPlayerServerId(v))
                                kills = kills + 1
                                Wait(500)
                                break
                            end
