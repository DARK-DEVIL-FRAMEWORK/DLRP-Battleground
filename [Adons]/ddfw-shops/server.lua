...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	TriggerEvent("ddfw-shops:MakeStash")
...
RegisterServerEvent('ddfw-shops:ShopOpen', function(shop, name, shoptable)
	local data = { shoptable = { products = shoptable.items, label = shoptable.label, }, custom = true }
	TriggerClientEvent('ddfw-shops:ShopMenu', source, data, true)
end)
...
RegisterServerEvent('ddfw-shops:GetItem', function(amount, billtype, item, shoptable, price, info, shop, num, nostash)
	local src = source
			...
			for i = 1, amount do -- Make a loop to put items into different slots rather than full amount in 1 slot
				if Player.Functions.AddItem(item, 1, nil, info) then
					if tonumber(i) == tonumber(amount) then -- when its on its last loop do this
						...
						TriggerClientEvent("ddfw-shops:SellAnim", src, {item = item, shoptable = shoptable})
					end
				else
		...
		if Config.Limit and not nostash then
			...
			for i = 1, #stashItems do
				if stashItems[i].name == item then
					...
					TriggerEvent('ddfw-shops:server:SaveStashItems', stashname, stashItems)
	...
	TriggerClientEvent('ddfw-shops:ShopMenu', src, data, custom)
end)

RegisterNetEvent("ddfw-shops:MakeStash", function()
	...
	for k, v in pairs(Config.Locations) do
		if k ~= "vendingmachine" then
			...
			if Config.Limit then TriggerEvent('ddfw-shops:server:SaveStashItems', "["..k.."("..l..")]", stashTable)
...
RegisterNetEvent("ddfw-shops:GenerateVend", function(data)
	local stashTable = {}
	...
	TriggerEvent('ddfw-shops:server:SaveStashItems', data[2], stashTable)
end)

--Compatability Wrapper Event for ddfw-truckerjob to refill shop stashes
RegisterNetEvent("ddfw-shops:server:RestockShopItems", function(storeinfo)
	local k, l = nil
	...
	if Config.Limit then TriggerEvent('ddfw-shops:server:SaveStashItems', "["..k.."("..l..")]", stashTable) end
end)

QBCore.Functions.CreateCallback('ddfw-shops:server:getLicenseStatus', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('ddfw-shops:server:sellChips', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('ddfw-shops:server:GetStashItems', function(source, cb, stashId) cb(GetStashItems(stashId)) end)
RegisterNetEvent('ddfw-shops:server:SaveStashItems', function(stashId, items) MySQL.Async.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', { ['stash'] = stashId, ['items'] = json.encode(items) }) end)
