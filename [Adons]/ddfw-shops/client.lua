	...
	for k, v in pairs(Config.Locations) do
		if k == "vendingmachine" and Config.VendOverride then
			for l, b in pairs(v["coords"]) do
				...
				exports['ddfw-target']:AddTargetModel(v["model"], {
					options = { {
						event = "ddfw-shops:ShopMenu", icon = (v["targetIcon"]), label = (v["targetLabel"]),
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { { event = "ddfw-shops:ShopMenu", icon = (v["targetIcon"] or "fas fa-cash-register"), label = (v["targetLabel"] or "Browse Shop"), item = v["requiredItem"], job = v["job"] or nil, gang = v["gang"] or nil,
				shoptable = v, name = v["label"], k = k, l = l, }, }
				if k == "casino" then
					options[#options+1] = { type = "server", event = "ddfw-shops:server:sellChips", icon = "fab fa-galactic-republic", label = "Trade Chips ($"..Config.SellCasinoChips.pricePer.." per chip)", }
				end
				Targets["Shop - ['"..k.."("..l..")']"] =
					exports['ddfw-target']:AddCircleZone("Shop - ['"..k.."("..l..")']", vector3(b.x, b.y, b.z), 2.0, { name="Shop - ['"..k.."("..l..")']", debugPoly=Config.Debug, useZ=true, },
...
RegisterNetEvent('ddfw-shops:ShopMenu', function(data, custom)
	...
	if Config.Limit and data.vend then
		...
		local p = promise.new()
		QBCore.Functions.TriggerCallback('ddfw-shops:server:GetStashItems', function(stash) p:resolve(stash) end, vendID)
		...
		if json.encode(stashItems) == "[]" then
			...
			TriggerServerEvent("ddfw-shops:GenerateVend", {data, vendID})
			Wait(1000)
			local p = promise.new()
			QBCore.Functions.TriggerCallback('ddfw-shops:server:GetStashItems', function(stash) p:resolve(stash) end, vendID)
			stashItems = Citizen.Await(p)
	...
	if Config.Limit and not custom and not data.vend then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('ddfw-shops:server:GetStashItems', function(stash) p:resolve(stash) end, "["..data.k.."("..data.l..")]")
		stashItems = Citizen.Await(p)
	end
	...
	ShopMenu[#ShopMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = "Close", params = { event = "ddfw-shops:CloseMenu" } }

	if data.shoptable["type"] == "weapons" then
		local p = promise.new()	local p2 = promise.new()
		QBCore.Functions.TriggerCallback("ddfw-shops:server:getLicenseStatus", function(hasLic, hasLicItem) p:resolve(hasLic) p2:resolve(hasLicItem) end)
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
			...
			for k, v in pairs(products[i].requiredJob) do
				if QBCore.Functions.GetPlayerData().job.name == k and QBCore.Functions.GetPlayerData().job.grade.level >= v then
					...
					if products[i].info ~= nil and products[i].info.image ~= nil then icon = "ddfw-inventory/html/images/"..products[i].info.image else icon = products[i].name end
					...
					ShopMenu[#ShopMenu + 1] = { icon = icon, header = header, txt = text, isMenuHeader = lock,
						params = { event = "ddfw-shops:Charge", args = { item = products[i].name, cost = products[i].price, info = products[i].info or {}, shoptable = data.shoptable, k = data.k, l = data.l, amount = amount, custom = custom } } }
					end
				end
			...
			elseif products[i].requiresLicense then
				...
				if products[i].info ~= nil and products[i].info.label ~= nil then header = products[i].info.label else header = setheader end
				ShopMenu[#ShopMenu + 1] = { icon = icon, header = header, txt = text, isMenuHeader = lock,
				params = { event = "ddfw-shops:Charge", args = {
	...
	exports['ddfw-menu']:openMenu(ShopMenu)
end)
--Selling animations are simply a pass item to seller animation
RegisterNetEvent('ddfw-shops:SellAnim', function(data)
...
RegisterNetEvent('ddfw-shops:CloseMenu', function() exports['ddfw-menu']:closeMenu() end)

RegisterNetEvent('ddfw-shops:Charge', function(data)
	...
	local dialog = exports['ddfw-input']:ShowInput({ header = header, submitText = "Pay", inputs = newinputs })
	if dialog then
		if not dialog.amount then return end
		if Config.Limit and data.custom == nil then	if tonumber(dialog.amount) > tonumber(data.amount) then TriggerEvent("QBCore:Notify", "Incorrect amount", "error") TriggerEvent("ddfw-shops:Charge", data) return end end
		if tonumber(dialog.amount) <= 0 then TriggerEvent("QBCore:Notify", "Incorrect amount", "error") TriggerEvent("ddfw-shops:Charge", data) return end
		if data.cost == "Free" then data.cost = 0 end
		if data.amount == nil then nostash = true end
		TriggerServerEvent('ddfw-shops:GetItem', dialog.amount, dialog.billtype, data.item, data.shoptable, data.cost, data.info, data.k, data.l or nil, nostash)
	end
end)

AddEventHandler('onResourceStop', function(resource) if resource ~= GetCurrentResourceName() then return end
	for k in pairs(Targets) do exports['ddfw-target']:RemoveZone(k) end
	for _, v in pairs(ped) do unloadModel(v) if IsModelAPed(GetEntityModel(v)) then DeletePed(v) else DeleteObject(v) end end
end)

-- local coords = vector3(161.27, -986.3, 30.09)
