        ...
        if default.matchInProgress then
            if #default.currentPlayers < Config.RequiredPlayers then -- if there are not enough players online, cancel the current match
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('ddfw-AssaultRifles:stop', default.currentPlayers[i].id)
            ...
            if #default.currentPlayers >= Config.RequiredPlayers then
                ...
                if default.queue.timer - os.time() <= 0 then
                    ...
                    for i = 1, #default.currentPlayers do
                        ...
                        TriggerClientEvent('ddfw-AssaultRifles:start', default.currentPlayers[i].id)
                    end
                end
            end
        end
    ...
    while true do
        ...
        if not default.matchInProgress then
            ...
            if default.matchEnds - os.time() <= Config.DisplayWinner then
                ...
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('ddfw-AssaultRifles:matchOver', default.currentPlayers[i].id, json.encode(winner), json.encode(default.currentPlayers[i]))
                end
                default = json.decode(json.encode(defaultDefault))
        ...
        TriggerClientEvent('ddfw-AssaultRifles:queueInfo', -1, text, other)
        Wait(500)
    end
end)

RegisterServerEvent('ddfw-AssaultRifles:join')
AddEventHandler('ddfw-AssaultRifles:join', function()
    local src = source
    ...
    if Player.Functions.RemoveMoney("cash", Config.Price) then
        ...
        TriggerClientEvent('ddfw-AssaultRifles:hudNotify', src, Config.Translations['no_money'])
    end
end)

RegisterServerEvent('ddfw-AssaultRifles:kill')
AddEventHandler('ddfw-AssaultRifles:kill', function(killed)
    local src = source
    if default.matchInProgress then
        ...
        if isplayerActive(killed) then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == killed then
                    ...
                    TriggerClientEvent('ddfw-AssaultRifles:die', default.currentPlayers[i].id)
                end
            end
        end
        if killedPlaying then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == src then
                    ...
                    TriggerClientEvent('ddfw-AssaultRifles:hudNotify', default.currentPlayers[i].id, Config.Translations['you_killed'] .. ': ~r~' .. GetPlayerName(killed))
                elseif default.currentPlayers[i].id == killed then
                    TriggerClientEvent('ddfw-AssaultRifles:hudNotify', default.currentPlayers[i].id, Config.Translations['you_got_killed'] .. ': ~r~' .. GetPlayerName(src))
                    TriggerClientEvent('ddfw-AssaultRifles:died', default.currentPlayers[i].id, src)
                else
                    TriggerClientEvent('ddfw-AssaultRifles:hudNotify', default.currentPlayers[i].id, GetPlayerName(killed) .. Config.Translations['killed_by'] .. GetPlayerName(src))
                end
            end
        end
    end
