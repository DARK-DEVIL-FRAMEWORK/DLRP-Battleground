...
RegisterNetEvent('ddfw-AssaultRifles:queueInfo')
AddEventHandler('ddfw-AssaultRifles:queueInfo', function(text, other)
    queueText = text
    matchends = other
end)

RegisterNetEvent('ddfw-AssaultRifles:hudNotify')
AddEventHandler('ddfw-AssaultRifles:hudNotify', function(text)
    SetNotificationTextEntry('STRING')
    AddTextComponentSubstringPlayerName(text)
    DrawNotification(false, true)
end)

RegisterNetEvent('ddfw-AssaultRifles:start')
AddEventHandler('ddfw-AssaultRifles:start', function()
    queuing = false 
...
RegisterNetEvent('ddfw-AssaultRifles:stop')
AddEventHandler('ddfw-AssaultRifles:stop', function()
...
RegisterNetEvent('ddfw-AssaultRifles:matchOver')
AddEventHandler('ddfw-AssaultRifles:matchOver', function(winner, me)
...
RegisterNetEvent('ddfw-AssaultRifles:died')
AddEventHandler('ddfw-AssaultRifles:died', function(killedBy)
    deaths = deaths + 1
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.JoinCircle, true) <= 7.5 then
                if queueText ~= Config.Translations['match_progress'] then
                    ...
                    if IsControlJustReleased(0, 38) then
                        queuing = not queuing
                        TriggerServerEvent('ddfw-AssaultRifles:join')
                    end
                else
                    ...
                    if GetEntityType(ped) == 1 then
                        for k, v in pairs(GetActivePlayers()) do
                            if GetPlayerPed(v) == ped then
                                TriggerServerEvent('ddfw-AssaultRifles:kill', GetPlayerServerId(v))
                                kills = kills + 1
                                Wait(500)
                                break
                            end
