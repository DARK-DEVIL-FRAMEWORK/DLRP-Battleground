...
function showBlur() {
    $.post('https://ddfw-inventory/showBlur');
}
...
function changeInventoryColor(color) {
    ...
    localStorage.setItem('ddfw-inventory-color', color);
}

const savedColor = localStorage.getItem('ddfw-inventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://ddfw-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://ddfw-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://ddfw-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://ddfw-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://ddfw-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://ddfw-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://ddfw-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://ddfw-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://ddfw-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://ddfw-inventory/CloseInventory", JSON.stringify({}));
