...
--- Event [Nui]
RegisterNetEvent('qb-admin:server:getMoney', function(moneytype)
    local src = source
    local result =  MySQL.query.await('SELECT money FROM players', {})
        if result then
            ...
            TriggerClientEvent('qb-admin:client:getMoney', src, totalMoney, moneytype)
        end
end)

RegisterNetEvent('qb-admin:server:getPlayerProfile', function(player)
    ...
    TriggerClientEvent('qb-admin:client:getPlayerProfile',source,data)
end)
