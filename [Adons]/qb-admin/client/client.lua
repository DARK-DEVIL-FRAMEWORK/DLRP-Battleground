...
RegisterNetEvent('qb-admin:client:getPlayerProfile', function(data)
    SendNUIMessage({type = "playerprofile", data = data})
end)

RegisterNetEvent('qb-admin:client:getMoney', function(totalmoney,moneytype)
    SendNUIMessage({type = "money", totalmoney = totalmoney, moneytype = moneytype})
end)


--- Callback [Nui]
RegisterNUICallback("qb-admin:nuicallback:getPlayers", function()
    QBCore.Functions.TriggerCallback('qb-admin:callback:getplayers', function(players)
        SendNUIMessage({type="getplayers", players = players})
    end)
end)

RegisterNUICallback("qb-admin:nuicallback:setMinPage", function()
    SetNuiFocusKeepInput(true)
    nuiWalks(true)
end)

RegisterNUICallback("qb-admin:nuicallback:getPlayerProfile", function(player)
    TriggerServerEvent('qb-admin:server:getPlayerProfile',player)
end)

RegisterNUICallback('qb-admin:nuicallback:getVehicles', function()
    SendNUIMessage({type="vehicles",vehicles=vehicles})
end)

RegisterNUICallback('qb-admin:nuicallback:getWeapons', function()
    SendNUIMessage({type="weapons",weapons=weapons})
end)

RegisterNUICallback('qb-admin:nuicallback:weather', function(weather)
...
RegisterNUICallback('qb-admin:nuicallback:time', function(time)
    TriggerServerEvent('qb-weathersync:server:setWeather', time, time)
    QBCore.Functions.Notify(Lang:t("time.changed", {time = time}))
end)

RegisterNUICallback("qb-admin:nuicallback:toggleMutePlayer", function(player)
...
RegisterNUICallback("qb-admin:nuicallback:event", function(data)
...
RegisterNUICallback("qb-admin:nuicallback:toFullPage", function()
    nuiWalks(false)
    SetNuiFocusKeepInput(false)
end)

RegisterNUICallback("qb-admin:nuicallback:escapeNui", function()
    nuiWalks(false)
    SetNuiFocusKeepInput(false)
