        ...
        if default.matchInProgress then
            if #default.currentPlayers < Config.RequiredPlayers then -- if there are not enough players online, cancel the current match
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('ddfw-smg:stop', default.currentPlayers[i].id)
            ...
            if #default.currentPlayers >= Config.RequiredPlayers then
                ...
                if default.queue.timer - os.time() <= 0 then
                    ...
                    for i = 1, #default.currentPlayers do
                        ...
                        TriggerClientEvent('ddfw-smg:start', default.currentPlayers[i].id)
                    end
                end
            end
        end
    ...
    while true do
        ...
        if not default.matchInProgress then
            ...
            if default.matchEnds - os.time() <= Config.DisplayWinner then
                ...
                for i = 1, #default.currentPlayers do
                    TriggerClientEvent('ddfw-smg:matchOver', default.currentPlayers[i].id, json.encode(winner), json.encode(default.currentPlayers[i]))
                end
                default = json.decode(json.encode(defaultDefault))
        ...
        TriggerClientEvent('ddfw-smg:queueInfo', -1, text, other)
        Wait(500)
    end
end)

RegisterServerEvent('ddfw-smg:join')
AddEventHandler('ddfw-smg:join', function()
    local src = source
    ...
    if Player.Functions.RemoveMoney("cash", Config.Price) then
        ...
        TriggerClientEvent('ddfw-smg:hudNotify', src, Config.Translations['no_money'])
    end
end)

RegisterServerEvent('ddfw-smg:kill')
AddEventHandler('ddfw-smg:kill', function(killed)
    local src = source
    if default.matchInProgress then
        ...
        if isplayerActive(killed) then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == killed then
                    ...
                    TriggerClientEvent('ddfw-smg:die', default.currentPlayers[i].id)
                end
            end
        end
        if killedPlaying then
            for i = 1, #default.currentPlayers do
                if default.currentPlayers[i].id == src then
                    ...
                    TriggerClientEvent('ddfw-smg:hudNotify', default.currentPlayers[i].id, Config.Translations['you_killed'] .. ': ~r~' .. GetPlayerName(killed))
                elseif default.currentPlayers[i].id == killed then
                    TriggerClientEvent('ddfw-smg:hudNotify', default.currentPlayers[i].id, Config.Translations['you_got_killed'] .. ': ~r~' .. GetPlayerName(src))
                    TriggerClientEvent('ddfw-smg:died', default.currentPlayers[i].id, src)
                else
                    TriggerClientEvent('ddfw-smg:hudNotify', default.currentPlayers[i].id, GetPlayerName(killed) .. Config.Translations['killed_by'] .. GetPlayerName(src))
                end
            end
        end
    end
