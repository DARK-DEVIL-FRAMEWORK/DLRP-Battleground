...
RegisterNetEvent('ddfw-smg:queueInfo')
AddEventHandler('ddfw-smg:queueInfo', function(text, other)
    queueText = text
    matchends = other
end)

RegisterNetEvent('ddfw-smg:hudNotify')
AddEventHandler('ddfw-smg:hudNotify', function(text)
    SetNotificationTextEntry('STRING')
    AddTextComponentSubstringPlayerName(text)
    DrawNotification(false, true)
end)

RegisterNetEvent('ddfw-smg:start')
AddEventHandler('ddfw-smg:start', function()
    queuing = false 
...
RegisterNetEvent('ddfw-smg:stop')
AddEventHandler('ddfw-smg:stop', function()
...
RegisterNetEvent('ddfw-smg:matchOver')
AddEventHandler('ddfw-smg:matchOver', function(winner, me)
...
RegisterNetEvent('ddfw-smg:died')
AddEventHandler('ddfw-smg:died', function(killedBy)
    deaths = deaths + 1
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.JoinCircle, true) <= 7.5 then
                if queueText ~= Config.Translations['match_progress'] then
                    ...
                    if IsControlJustReleased(0, 38) then
                        queuing = not queuing
                        TriggerServerEvent('ddfw-smg:join')
                    end
                else
                    ...
                    if GetEntityType(ped) == 1 then
                        for k, v in pairs(GetActivePlayers()) do
                            if GetPlayerPed(v) == ped then
                                TriggerServerEvent('ddfw-smg:kill', GetPlayerServerId(v))
                                kills = kills + 1
                                Wait(500)
                                break
                            end
