...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        if (GetResourceState("ddfw-weathersync") !== "started") return interaction.reply({ content: "This command requires QBCore's `ddfw-weathersync` to work", ephemeral: false });
        if (args.blackout) {
            // doesn't give any option for true or false or feedback to which was done -.-
            emit("ddfw-weathersync:server:toggleBlackout");
            ...
            emit("ddfw-weathersync:server:setWeather", args.weather);
