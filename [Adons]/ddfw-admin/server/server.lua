...
--- Event [Nui]
RegisterNetEvent('ddfw-admin:server:getMoney', function(moneytype)
    local src = source
    local result =  MySQL.query.await('SELECT money FROM players', {})
        if result then
            ...
            TriggerClientEvent('ddfw-admin:client:getMoney', src, totalMoney, moneytype)
        end
end)

RegisterNetEvent('ddfw-admin:server:getPlayerProfile', function(player)
    ...
    TriggerClientEvent('ddfw-admin:client:getPlayerProfile',source,data)
end)
