...
QBCore.Functions.CreateCallback('test:getdealers', function(_, cb)
    cb(exports['ddfw-drugs']:GetDealers())
end)

QBCore.Functions.CreateCallback('ddfw-admin:callback:getplayers', function(_, cb)
    cb(players)
end)

QBCore.Functions.CreateCallback('ddfw-admin:server:getrank', function(source, cb)
...
local function BanPlayer(src)
    ...
    TriggerEvent('ddfw-admin:log:playersEvent',src,"cheater","Banned Cheater \n Trying to trigger admin options which they dont have permission for","red",src)
    DropPlayer(src, 'You were permanently banned by the server for: Exploiting')
end

-- Events
RegisterNetEvent('ddfw-admin:server:GetPlayersForBlips', function()
    local src = source
    TriggerClientEvent('ddfw-admin:client:Show', src, players)
end)

RegisterNetEvent('ddfw-admin:server:kill', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['kill']) or IsPlayerAceAllowed(src, 'command')  then
        ...
        TriggerEvent('ddfw-admin:log:playersEvent',src,"kill","Killed".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:revive', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['revive']) or IsPlayerAceAllowed(src, 'command')  then
        ...
        TriggerEvent('ddfw-admin:log:playersEvent',src,"revivep","Revive".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:kick', function(data)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['kick']) or IsPlayerAceAllowed(src, 'command')  then
        TriggerEvent('ddfw-admin:log:playersEvent',src,"kick","Kicked".."["..data[3].."] \n Reason: "..data[4],"yellow",data[3])
...
RegisterNetEvent('ddfw-admin:server:ban', function(data)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['ban']) or IsPlayerAceAllowed(src, 'command') then
        ...
        TriggerEvent('ddfw-admin:log:playersEvent',src,"ban","Banned".."["..data[3].."] \n Reason: "..data[5],"red",data[3])
        if banTime >= 2147483647 then
...
RegisterNetEvent('ddfw-admin:server:spectate', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['spectate']) or IsPlayerAceAllowed(src, 'command') then
        local targetped = GetPlayerPed(player)
        local coords = GetEntityCoords(targetped)
        TriggerClientEvent('ddfw-admin:client:spectate', src, player, coords)
        TriggerEvent('ddfw-admin:log:playersEvent',src,"spectate","Spectate".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:freeze', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['freeze']) or IsPlayerAceAllowed(src, 'command') then
        TriggerEvent('ddfw-admin:log:playersEvent',src,"freeze","Freeze".."["..player.."]","black",player)
...
RegisterNetEvent('ddfw-admin:server:goto', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['goto']) or IsPlayerAceAllowed(src, 'command') then
        local admin = GetPlayerPed(src)
        local coords = GetEntityCoords(GetPlayerPed(player))
        SetEntityCoords(admin, coords)
        TriggerEvent('ddfw-admin:log:playersEvent',src,"gotobring","Goto".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:intovehicle', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['intovehicle']) or IsPlayerAceAllowed(src, 'command') then
        ...
        if vehicle ~= 0 then
            ...
            if seat ~= -1 then
                ...
                TriggerEvent('ddfw-admin:log:playersEvent',src,"intovehicle","Into vehicle".."["..player.."]","black",player)
            else
...
RegisterNetEvent('ddfw-admin:server:bring', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['bring']) or IsPlayerAceAllowed(src, 'command') then
        ...
        TriggerEvent('ddfw-admin:log:playersEvent',src,"gotobring","Bring".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:inventory', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['inventory']) or IsPlayerAceAllowed(src, 'command') then
        TriggerClientEvent('ddfw-admin:client:inventory', src, player)
        TriggerEvent('ddfw-admin:log:playersEvent',src,"inventory","Open Inventory".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:cloth', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['clothing']) or IsPlayerAceAllowed(src, 'command') then
        TriggerClientEvent('ddfw-clothing:client:openMenu', player)
        TriggerEvent('ddfw-admin:log:playersEvent',src,"clothing","Open Clothing".."["..player.."]","black",player)
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:setPermissions', function(data)
    local src = source
    if QBCore.Functions.HasPermission(src, 'god') or IsPlayerAceAllowed(src, 'command') then
        ...
        TriggerEvent('ddfw-admin:log:playersEvent',src,"perms","Perm: ["..data[4].rank.."]", "black",data[3])
    else
        BanPlayer(src)
    end
end)

RegisterNetEvent('ddfw-admin:server:SendReport', function(name, targetSrc, msg)
    local src = source
...
RegisterServerEvent('ddfw-admin:giveWeapon', function(weapon)
    local src = source
    if QBCore.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
...
RegisterServerEvent('ddfw-admin:server:givemoneyadmin', function(data)
    local src = source
    if QBCore.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
        local Player = QBCore.Functions.GetPlayer(tonumber(data[3]))
        if Player then
            Player.Functions.AddMoney(tostring(data[4].money), tonumber(data[5]))
            TriggerEvent('ddfw-admin:log:playersEvent',src,"givemoney","Give Money".."[Type:"..tostring(data[4].money).."] Total:"..tonumber(data[5]),"black",tonumber(data[3]))
        else
...
RegisterNetEvent('ddfw-admin:server:SaveCar', function(mods, vehicle, _, plate)
    local src = source
    if QBCore.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
        local Player = QBCore.Functions.GetPlayer(src)
