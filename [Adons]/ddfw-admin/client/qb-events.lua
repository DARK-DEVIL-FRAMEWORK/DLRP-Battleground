...
RegisterNetEvent('ddfw-admin:client:toggleshowcoords', function()
    local x = 0.4
    local y = 0.025
...
RegisterNetEvent('ddfw-admin:client:togglevehicledev', function()
    local x = 0.4
    local y = 0.888
...
RegisterNetEvent('ddfw-admin:client:freeaim', function()
    ToggleEntityFreeView()
end)

RegisterNetEvent('ddfw-admin:client:viewmode', function(mode)
...
RegisterNetEvent('ddfw-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('ddfw-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('ddfw-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('ddfw-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('ddfw-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('ddfw-admin:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
            TriggerServerEvent('ddfw-admin:log:minPage',"vehicle","savecar","blue")
        else
...
RegisterNetEvent('ddfw-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
local speed = false
RegisterNetEvent('ddfw-admin:client:SetSpeed', function()
    local ped = PlayerId()
    speed = not speed
    TriggerServerEvent('ddfw-admin:log:minPage',"other","fast speed","blue")
    if speed then
...
RegisterNetEvent("ddfw-admin:client:reviveSelf", function()
    TriggerEvent('hospital:client:Revive', PlayerPedId())
    TriggerServerEvent('ddfw-admin:log:minPage',"revive","revive","black")
end)

local devmode = false
RegisterNetEvent("ddfw-admin:client:devMode", function()
    devmode = not devmode
    TriggerEvent('ddfw-admin:client:godMode')
    TriggerEvent('ddfw-admin:client:infiniteAmmo')
    TriggerEvent('ddfw-admin:client:SetSpeed')
    QBCore.Functions.Notify("Godmode,Infinite Ammo,Fast Speed | "..tostring(devmode))
end)

local superJump = false
RegisterNetEvent("ddfw-admin:client:superJump", function()
...
local godmode = false
RegisterNetEvent("ddfw-admin:client:godMode", function()
    godmode = not godmode
    TriggerServerEvent('ddfw-admin:log:minPage',"godmode","godmode","black")
    if godmode then
        while godmode do
            Wait(0)
...
RegisterNetEvent("ddfw-admin:client:infiniteAmmo", function()
    InfiniteAmmo = not InfiniteAmmo
    TriggerServerEvent('ddfw-admin:log:minPage',"other","infinite ammo","blue")
    local ped = PlayerPedId()
...
RegisterNetEvent("ddfw-admin:client:invisible", function()
    invisible = not invisible
    TriggerServerEvent('ddfw-admin:log:minPage',"invisible","invisible","black")
    if invisible then
...
RegisterNetEvent("ddfw-admin:client:vehicleGodMode", function()
    VehicleGodmode = not VehicleGodmode
    TriggerServerEvent('ddfw-admin:log:minPage',"vehicle","veh godmode","blue")
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
...
RegisterNetEvent('ddfw-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = PlayerPedId()
...
RegisterNetEvent('ddfw-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('ddfw-admin:client:maxmodVehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId())
    PerformanceUpgradeVehicle(vehicle)
    TriggerServerEvent('ddfw-admin:log:minPage',"vehicle","maxmods","blue")
end)

RegisterNetEvent('ddfw-admin:client:copyToClipboard', function(dataType)
