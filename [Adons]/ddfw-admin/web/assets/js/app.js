const adminPanel = Vue.createApp({
    ...
    methods: {
	   ...
	   this.minpage = minPage
       	   this.devs = devs
	   this.fetchNui(`ddfw-admin:nuicallback:${callBack}`, '')
        },
        ...
        actionEvent(data) {
            this.fetchNui('ddfw-admin:nuicallback:event', data)
        },
        ...
        setDistance() {
            ...
            this.fetchNui('ddfw-admin:client:viewdistance',this.distance)
        },
        keyupHandler(e) {
            if (e.key == "Escape") {
                this.minpage = false;
                this.devs = false;
                this.panel = false;
                this.fetchNui('ddfw-admin:nuicallback:escapeNui', '')
            } else if (e.which == 32) {
                e.preventDefault();
            }
        },
