...
local function SaveSkin()
    ...
    TriggerServerEvent("ddfw-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    TriggerServerEvent("ddfw-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("ddfw-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    ...
    QBCore.Functions.TriggerCallback('ddfw-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('ddfw-clothing:client:openMenu')
AddEventHandler('ddfw-clothing:client:openMenu', function()
...
RegisterNetEvent('ddfw-clothing:client:openMenu-barber')
AddEventHandler('ddfw-clothing:client:openMenu-barber', function()
    customCamLocation = nil
    openMenu({
...
RegisterNetEvent('ddfw-clothing:client:reloadOutfits')
AddEventHandler('ddfw-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('ddfw-clothes:client:CreateFirstCharacter')
AddEventHandler('ddfw-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("ddfw-clothes:loadSkin")
AddEventHandler("ddfw-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('ddfw-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)
RegisterNetEvent('ddfw-clothing:client:loadPlayerClothing')
AddEventHandler('ddfw-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('ddfw-clothing:client:loadOutfit')
AddEventHandler('ddfw-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("ddfw-clothing:client:adjustfacewear")
AddEventHandler("ddfw-clothing:client:adjustfacewear",function(type)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ddfw-clothes:loadPlayerSkin")
    PlayerData = QBCore.Functions.GetPlayerData()
--    QBCore.Shared.Jobs = exports['ddfw-jobs']:AddJobs()
end)
...
RegisterNetEvent('ddfw-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('ddfw-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('ddfw-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('ddfw-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('ddfw-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('ddfw-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports['ddfw-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            exports['ddfw-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
                ...
                options = {
                    {
                        type = "client",
                        event = "ddfw-clothing:client:openMenu",
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['ddfw-core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['ddfw-core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['ddfw-core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false
                exports['ddfw-core']:HideText()
            end
        end)
