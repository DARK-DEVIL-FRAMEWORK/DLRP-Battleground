...
QBCore.Commands.Add('me', 'Me message.', {}, false, function(source, args)
	...
	if (not args:find("<") and not args:find(">")) or GetPlayerName(source) == "MHMori" then
		...
		TriggerEvent('ddfw-logs:server:createLog', GetCurrentResourceName(), GetCurrentResourceName(), '**ME** ' .. args, source)
	end
end)
...
QBCore.Commands.Add('do', 'Do message.', {}, false, function(source, args)
	...
	if (not args:find("<") and not args:find(">")) or GetPlayerName(source) == "MHMori" then
		...
		TriggerEvent('ddfw-logs:server:createLog', GetCurrentResourceName(), GetCurrentResourceName(), '**DO** ' .. args, source)
	end
end)
