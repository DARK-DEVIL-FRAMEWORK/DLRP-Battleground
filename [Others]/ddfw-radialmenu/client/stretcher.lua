...
local function LayOnStretcher()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('ddfw-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(player), "lay")
        else
            loadAnim(inBedDicts)
            if stretcherObject and #(coords - GetEntityCoords(stretcherObject)) <= 3.0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
local function attachToStretcher()
    ...
    if stretcherObject then
        if closestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('ddfw-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(closestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(stretcherObject)
                loadAnim("anim@heists@box_carry@")
                TaskPlayAnim(ped, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
...
RegisterNetEvent('ddfw-radialmenu:client:TakeStretcher', function()
...
RegisterNetEvent('ddfw-radialmenu:client:RemoveStretcher', function()
    local ped = PlayerPedId()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            if DoesEntityExist(stretcherObject) then
                ...
                ClearPedTasks(ped)
                DetachEntity(ped, false, true)
                TriggerServerEvent('ddfw-radialmenu:server:RemoveStretcher', coords, stretcherObject)
                isAttached = false
                stretcherObject = nil
                isLayingOnBed = false
            end
...
RegisterNetEvent('ddfw-radialmenu:client:RemoveStretcherFromArea', function(playerPos, bObject)
    local ped = PlayerPedId()
...
RegisterNetEvent('ddfw-radialmenu:Stretcher:client:BusyCheck', function(otherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
        loadAnim("anim@gangops@morgue@table@")
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('ddfw-radialmenu:server:BusyResult', true, otherId, type)
        else
            TriggerServerEvent('ddfw-radialmenu:server:BusyResult', false, otherId, type)
        end
    else
        loadAnim('anim@heists@box_carry@')
...
RegisterNetEvent('ddfw-radialmenu:client:Result', function(isBusy, type)
    local ped = PlayerPedId()
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    if type == "lay" then
