...
local function ShowEntranceHeaderMenu()
    ...
    exports['ddfw-menu']:showHeader(headerMenu)
end
...
local function ShowExitHeaderMenu()
    ...
    exports['ddfw-menu']:openMenu(headerMenu)
end
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside and text then
            if targetKey == "entrancePos" then
                ...
                exports['ddfw-core']:DrawText(text, 'left')
            end
        else
            if targetKey == "entrancePos" then
                CloseMenuFull()
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['ddfw-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                RangDoorbell = nil
                Wait(500)
                TriggerEvent('ddfw-weathersync:client:DisableSync')
                Wait(100)
                TriggerServerEvent('ddfw-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('ddfw-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('ddfw-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("apartments:server:setCurrentApartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('ddfw-interior:client:SetNewState', true)
            else
                TriggerEvent('ddfw-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('ddfw-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("ddfw-apartments:returnBucket")
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(10) end
    exports['ddfw-interior']:DespawnInterior(HouseObj, function()
        TriggerEvent('ddfw-weathersync:client:EnableSync')
        ...
        TriggerServerEvent('ddfw-apartments:server:SetInsideMeta', CurrentApartment, false)
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            ...
            exports['ddfw-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('ddfw-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('ddfw-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('ddfw-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('ddfw-spawn:client:openUI', true)
            else
                TriggerEvent('ddfw-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('ddfw-spawn:client:openUI', true)
...
RegisterNetEvent('ddfw-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:ChangeOutfit', function()
    ...
    TriggerEvent('ddfw-clothing:client:openOutfitMenu')
end)

RegisterNetEvent('apartments:client:Logout', function()
    TriggerServerEvent('ddfw-houses:server:LogoutLocation')
end)
