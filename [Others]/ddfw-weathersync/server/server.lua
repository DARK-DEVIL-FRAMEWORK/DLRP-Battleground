...
local function nextWeatherStage()
    ...
    TriggerEvent("ddfw-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('ddfw-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('ddfw-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('ddfw-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('ddfw-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('ddfw-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('ddfw-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('ddfw-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('ddfw-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
